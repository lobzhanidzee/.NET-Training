namespace Exceptions
{
    public static class ThrowingArgumentOutOfRange
    {
        ...
        public static bool CheckParametersAndThrowException4(long l, float f)
        {
            if (l < -9 || l >= 9 || (l >= -3 && l < 3))
            {
                throw new ArgumentOutOfRangeException(nameof(l), $"{nameof(l)} should be in [-9, -3) or [3, 9) intervals.");
            }
            else if (f <= -0.3f || f >= 0.3f)
            {
                throw new ArgumentOutOfRangeException(nameof(f), $"{nameof(f)} should be in the (-0.3, 0.3) interval.");
            }
